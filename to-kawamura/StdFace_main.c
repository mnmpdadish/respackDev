...
static void StdFace_ResetVals(struct StdIntList *StdI) {
  ...
  */
  ...
  for (i = 0; i < 3; i++)StdI->cutoff_DR[i] = StdI->NaN_i; /*Kazuma Nakamura*/ 
#if defined(_HPhi)
...
)
{
  ...
  while (fgets(ctmpline, 256, fp) != NULL) {
    ...
    /*Kazuma Nakamura*/
    else if (strcmp(keyword, "cutoff_d") == 0) StoreWithCheckDup_d(keyword, value, &StdI->cutoff_d);
    else if (strcmp(keyword, "cutoff_dh") == 0) StoreWithCheckDup_i(keyword, value, &StdI->cutoff_DR[2]);
    else if (strcmp(keyword, "cutoff_dl") == 0) StoreWithCheckDup_i(keyword, value, &StdI->cutoff_DR[1]);
    else if (strcmp(keyword, "cutoff_dw") == 0) StoreWithCheckDup_i(keyword, value, &StdI->cutoff_DR[0]);
    /*Kazuma Nakamura*/
