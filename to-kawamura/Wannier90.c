...
)
{
  ...
  */
  ...
  /*
  Read DMX Kazuma Nakamura 
  */
  fprintf(stdout, "\n  @ Wannier90 DMX \n\n");
  StdFace_PrintVal_d("cutoff_d", &StdI->cutoff_d, 1.0e-8);
  StdFace_PrintVal_d("cutoff_length_D", &StdI->cutoff_length_D, -1.0);
  sprintf(filename, "%s_dr.dat", StdI->CDataFileHead);
  n_d = read_W90_query(StdI, filename, 
    StdI->cutoff_d, StdI->cutoff_DR, StdI->cutoff_length_D);
  W90_d = (double complex *)malloc(sizeof(double complex) * n_d);
  d_indx = (int **)malloc(sizeof(int*) * n_d);
      ...
      */
        ...
        && t_indx[it][3] == t_indx[it][4])
      {
        if (strcmp(StdI->model, "hubbard") == 0) {
          isite = StdI->NsiteUC*kCell + t_indx[it][3];
          for (ispin = 0; ispin < 2; ispin++) {
            StdI->trans[StdI->ntrans] = -W90_t[it]+delta[isite];
            StdI->transindx[StdI->ntrans][0] = isite;
            StdI->transindx[StdI->ntrans][1] = ispin;
            StdI->transindx[StdI->ntrans][2] = isite;
            StdI->transindx[StdI->ntrans][3] = ispin;
