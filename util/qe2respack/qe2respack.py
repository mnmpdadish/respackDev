        ...
        if oldxml:
            for k in range(num_k):
                if(not is_SpinOrbit):
                    ...
                    with Iotk_dat(os.path.join(dirname, 'K{0:0>5}/evc1.dat'.format(k+1)), endian=endian) as inp1:
                      with Iotk_dat(os.path.join(dirname, 'K{0:0>5}/evc2.dat'.format(k+1)), endian=endian) as inp2:
                        for ib in range(num_b):
