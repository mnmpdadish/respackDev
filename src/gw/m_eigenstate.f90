...
contains
  ...
  subroutine calculate_qpvalue(NWF,ncalck,Na1,Na2,Na3,nkb1,nkb2,nkb3,flg_weight,a1,a2,a3,SK0,QPR,EQP) 
    implicit none 
    integer,intent(in)::NWF,ncalck,Na1,Na2,Na3,nkb1,nkb2,nkb3 
    integer,intent(in)::flg_weight 
    real(8),intent(in)::a1(3),a2(3),a3(3)
    real(8),intent(in)::SK0(3,ncalck) 
    complex(8),intent(in)::QPR(NWF,NWF,-Na1:Na1,-Na2:Na2,-Na3:Na3)   
    !--
    !out
    complex(8),intent(out)::EQP(NWF,ncalck)           
    !-- 
    real(8),allocatable::WEIGHT_R(:,:,:)!WEIGHT_R(-Na1:Na1,-Na2:Na2,-Na3:Na3)
    complex(8),allocatable::pf(:,:,:,:)!pf(-Na1:Na1,-Na2:Na2,-Na3:Na3,ncalck)   
    integer::ia1min,ia2min,ia3min 
    ...
    !
    EQP=0.0d0 
    call make_emk(ncalck,NWF,Na1,Na2,Na3,QPR(1,1,-Na1,-Na2,-Na3),pf(-Na1,-Na2,-Na3,1),EQP(1,1)) 
    !
    deallocate(WEIGHT_R,pf) 
    !
    return 
  ...
  subroutine make_emk(NTK,NWF,Na1,Na2,Na3,HmatT,pf,EMK)  
    implicit none 
    integer::NTK,NWF,Na1,Na2,Na3 
    complex(8)::pf(-Na1:Na1,-Na2:Na2,-Na3:Na3,NTK)   
    complex(8)::HmatT(NWF,NWF,-Na1:Na1,-Na2:Na2,-Na3:Na3)   
    complex(8)::larger_en
    ...
    integer::ik,ib,jb,ia1,ia2,ia3,i,j
    complex(8)::SUM_CMPX 
    complex(8)::EMK(NWF,NTK)           
    !
     ...
     do ib=1,NWF      
      do jb=1,NWF      
       ...
       do ia1=-Na1,Na1 
        do ia2=-Na2,Na2 
         do ia3=-Na3,Na3 
          SUM_CMPX=SUM_CMPX+HmatT(ib,jb,ia1,ia2,ia3)*pf(ia1,ia2,ia3,ik)  
         enddo!ia3            
        enddo!ia2                       
       enddo!ia1
       Hin(ib,jb)=SUM_CMPX           
