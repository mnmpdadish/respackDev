...
contains
  ...
  subroutine calculate_banddisp(NWF,NSK_BAND_DISP,Ndiv,N_sym_points,threshold_transfer,b1,b2,b3,SK_BAND_DISP,EN) 
    implicit none 
    ...
    real(8),intent(in)::EN(NWF,NSK_BAND_DISP) 
    !
    ...
    !wrt dat.iband 
    !
    !call wrt_iband(threshold_transfer,NWF,NSK_BAND_DISP,kdata(1),EN(1,1)) 
    !
    !wrt dat.qpband 
    !
    call wrt_qpband(threshold_transfer,NWF,NSK_BAND_DISP,kdata(1),EN(1,1)) 
    !
    return
  end subroutine      
  ...
  subroutine wrt_qpband(threshold_transfer,NWF,NSK_BAND_DISP,kdata,EQP) 
    implicit none
    integer,intent(in)::NWF,NSK_BAND_DISP
    real(8),intent(in)::kdata(NSK_BAND_DISP)
    real(8),intent(in)::EQP(NWF,NSK_BAND_DISP) 
    real(8),intent(in)::threshold_transfer 
    integer::ib,ik 
    LOGICAL::REVERSE 
    real(8),parameter::au=27.21151d0 
    !
    !OPEN(114,W,FILE='./dat.qpband') 
    !
    OPEN(114,FILE='./dat.qpband') 
    write(114,'(a)')'#Wannier-interpolaed quasi-particle band'
    write(114,'(a,x,f10.5)')'#Energy cutoff for transfer (eV)=',threshold_transfer*au
    write(114,'(a)')'#1:k, 2:Energy [eV]' 
    REVERSE=.TRUE.        
    do ib=1,NWF 
     if(REVERSE)then 
      do ik=1,NSK_BAND_DISP                     
       write(114,'(2f20.10)') kdata(ik),EQP(ib,ik)*au
      enddo!ik        
      REVERSE=.FALSE.        
     else         
      do ik=NSK_BAND_DISP,1,-1          
